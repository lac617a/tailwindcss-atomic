...
const BODY =
	"M32 12 H76 A6 6 0 0 1 82 18 V60 A16 16 0 0 1 66 76 H26 A8 8 0 0 1 18 68 V26 A14 14 0 0 1 32 12 Z";
	...
	if (kind === "happy") {
		return (
			<path
				...
				d={`M${cx - 8} ${EYE_Y + 4} Q${cx} ${EYE_Y - 8} ${cx + 8} ${EYE_Y + 4}`}
			/>
		);
	}
	if (kind === "line") {
		return (
			<path className={styles.eyeLine} d={`M${cx - 8} ${EYE_Y} H${cx + 8}`} />
		);
	}
				...
				<g className={styles.figure}>
					<g className={styles.limbs}>
						<rect
							...
							height={9}
							rx={4.5}
							transform={`rotate(${armL} 20 ${SHOULDER_Y})`}
						/>
						{armR === "wave" ? (
							<rect
							...
							<rect
								...
								height={9}
								rx={4.5}
								transform={`rotate(${armR} 80 ${SHOULDER_Y})`}
							/>
						)}
					...
					<g
						...
						style={{transform: `translateY(${browDy}px)`}}
					>
						<rect
							x={30}
							y={24}
							...
							height={4}
							rx={2}
							transform={`rotate(${browL} 37.5 26)`}
						/>
						<rect
							x={55}
							y={24}
							...
							height={4}
							rx={2}
							transform={`rotate(${browR} 62.5 26)`}
						/>
					</g>
					...
					{state === "solving" && (
						<path
							...
							d="M72 20 L78 31 A6 6 0 0 1 66 31 Z"
						/>
					)}
				</g>
				...
				<g className={styles.fx}>
					...
					{state === "sleep" &&
						[0, 1, 2].map((i) => (
							<g
								key={`z${i}`}
								transform={`translate(${82 + i * 5} ${16 - i * 6})`}
							>
								<text
							...
							{sparkSpots.map(([x, y], i) => (
								<g key={`s${i}`} transform={`translate(${x} ${y})`}>
									<path
										...
										style={{
											fill: cubeColors[i % cubeColors.length],
