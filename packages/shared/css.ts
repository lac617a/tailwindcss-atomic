...
/**
 * Tailwind class grammar (keep in sync with `src/tailwind.rs`).
 * There is no closed catalog: arbitrary values, stacked variants and
 * `@theme` tokens are generated at compile time. A utility is a bare
 * word, a known core-plugin prefix, or an `[arbitrary]` / `variant:` form.
 * Custom BEM classes (`.header-signin`, `.btn-notch`) do not match.
 */
...
const EXACT_TAILWIND_UTILITIES = new Set([
	"border-collapse",
	"border-separate",
	"box-border",
	"box-content",
...
function looksLikeTailwindCore(core: string) {
	if (!core) return false;
	if (core.includes("[")) return true;
	const unsigned = core.startsWith("-") ? core.slice(1) : core;
	const base = unsigned.split("/")[0] ?? unsigned;
	if (BARE_TAILWIND_UTILITIES.has(base) || EXACT_TAILWIND_UTILITIES.has(base)) {
		return true;
	}
	return TAILWIND_UTILITY_PREFIXES.some(
		(prefix) => base.startsWith(prefix) && base.length > prefix.length,
	);
}
...
function looksLikeTailwindUtilityClass(className: string) {
	...
	return looksLikeTailwindCore(coreUtilityName(name));
}
...
/**
 * Utilidades Tailwind (`.flex`, `.py-2`, `.hover\:bg-red-500:hover`, `--tw-*`).
 * No: tokens de skin, componentes custom con guion (`.header-signin`, `.btn-notch`),
 * SCSS (`.a .b`, `.pattern-background::before`) ni `:root` / `html` / `[data-theme]`.
