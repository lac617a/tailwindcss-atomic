...
async function warmupCssFile(
	...
	if (twPostcss) {
		...
		const result = await postcss(plugins as AcceptedPlugin[]).process(
			prepared,
			{
				from: cssPath,
			},
		);
		applyAtomicCss(result.css, cssPath);
		return;
	}
	...
	const v3Plugins = [twV3];
	const v3Autoprefixer = asPostcssPlugin(
		tryRequire(appRequire, "autoprefixer"),
	);
	if (v3Autoprefixer) v3Plugins.push(v3Autoprefixer);
	const v3Result = await postcss(v3Plugins as AcceptedPlugin[]).process(
		prepared,
		{from: cssPath},
	);
	applyAtomicCss(v3Result.css, cssPath);
}
