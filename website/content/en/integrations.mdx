...
Do **not** hash that UI `dist` in Rollup (`preserveModules` should keep original utilities). Do **not** pass `{ library: true }` to `withTailwindAtomic` in the Next app — that option is only for the design-system Rollup build, and the Next adapter ignores it.
