...
<Callout>
  Wrappers that return a Next **config function** (`withPostHogConfig`, Sentry, some `next-intl` setups) must be invoked, not spread. `withTailwindcssAtomic` now unwraps those functions so your `webpack()` fallbacks stay intact. PostHog still wants to be the **outermost** wrapper:
...
`withTailwindcssAtomic` injects the Webpack loader (pre) and the plugin that, in `processAssets`, atomicizes **all** CSS and then rewrites **all** JS (including server chunks). In `next dev` it invalidates JS modules when the map changes.
...
`withTailwindcssAtomic` fills `turbopack.rules` with the TS/JS loader (plus the Webpack hook). In Next 16.3+ rules are an array with `condition: "foreign"` and `condition: { not: "foreign" }`; the `{ foreign, default }` shorthand no longer validates. Tailwind 4 PostCSS is still required.
...
Do **not** hash that UI `dist` in Rollup (`preserveModules` should keep original utilities). Do **not** pass `{ library: true }` to `withTailwindcssAtomic` in the Next app — that option is only for the design-system Rollup build, and the Next adapter ignores it.

If `bg-opacity-50` is in the DOM but there is no `background-color`, `twMerge` v3 dropped `bg-neutral-300`. Use `bg-neutral-300/50`. See [Preserve & runtime](./preserve).
