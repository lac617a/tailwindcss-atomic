...
**No** hashees ese `dist` del UI en Rollup (`preserveModules` debe dejar las utilidades originales). **No** pases `{ library: true }` a `withTailwindAtomic` en la app Next: esa opción es solo para el Rollup del design system, y el adapter de Next la ignora.
